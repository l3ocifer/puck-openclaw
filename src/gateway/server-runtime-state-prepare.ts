...
import { createA2aIngressHandler } from "./server-a2a-ingress.js";
...
export async function prepareGatewayRuntimeState(params: {
  ...
  const {
    ...
    createGatewayRuntimeState({
      ...
      // Homelab patch `a2a-gateway-ingress` — see server-a2a-ingress.ts.
      handleA2aRequest: createA2aIngressHandler({
        getGatewayRequestContext: () => pluginGatewayContext.current,
      }),
